...
@process_clause_header.register(fastobo.header.TreatXrefsAsReverseGenusDifferentiaClause)
def _process_clause_header_treat_xrefs(clause, meta):
    warnings.warn(
        f"cannot process `{clause}` macro",
...
@functools.singledispatch
def process_clause_typedef(clause: fastobo.typedef.BaseTypedefClause, rel: RelationshipData):
