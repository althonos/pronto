...
from pronto.utils.io import decompress, get_handle
...
class TurtleParser(BaseParser):
    """A parser for OWL2 ontologies serialized in TTL format.
    ...
    def can_parse(cls, path, buffer):
        try:
            g = Graph()
            g.parse(path)
            g.serialize(destination=f"{path.strip('.ttl')}.owl", format="pretty-xml")
            TurtleParser.path = f"{path.strip('.ttl')}.owl"

        except Exception as e:
            print("Some error in parsing or serializing the file")
            print(f"Exception = {e}")
            return False
        return True
    ...
    def parse_from(self, handle, threads=None):
        ...
        with contextlib.ExitStack() as ctx:
            self.path = handle = os.fspath(self.path)
            self.handle = ctx.enter_context(get_handle(handle, timeout = 5))
            _handle = ctx.enter_context(decompress(self.handle))
            self.handle = _handle
            print(f"Handle is {self.handle} of type {type(self.handle)}")
            tree: etree.ElementTree = etree.parse(self.handle)
